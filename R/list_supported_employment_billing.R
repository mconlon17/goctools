...
list_supported_employment_billing <- function(date = Sys.Date()) {
    ...
    members <- members %>%
        ...
        dplyr::summarise(DischargeDate = max(lubridate::as_date(flo_goal_end_date), na.rm = T), 
                         EmploymentPlanUpdate = max(date_modified, na.rm = T)) %>%
        dplyr::mutate(DischargeDate = lubridate::as_date(ifelse(DischargeDate == -Inf, NA, DischargeDate)), 
