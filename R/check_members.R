...
check_members <- function(check = "contacts", active.only = T) {
  ...
  if (check == "contacts") {
      tab <- members %>%
          dplyr::select(Name, flo_primary_phone, primary_address_street, primary_address_city, primary_address_state, 
  ...
  if (check == "emergency") {
      tab <- members %>%
          dplyr::select(Name, flo_emergency_con_phone_c, emergency_address_street, emergency_address_city, emergency_address_state, 
  ...
  if (check == "demographics") {
      tab <- members %>%
          dplyr::select(Name, flo_deceased_c, birthdate,flo_gender_at_birth_c, flo_gender_identity_c, flo_preferred_pronouns_c,
  ...
  if (check == "medical") {
      tab <- members %>%
          dplyr::select(Name, flo_diagnoses_mult, flo_diagnosis_comments, flo_accessibility_restrictions, 
                 flo_allergies)
  ...
  if (check == "admin") {
      tab <- members %>%
          dplyr::select(Name, flo_membership_status_c, flo_transportation_mode_c, flo_unit_chosen_by_member_c, 
