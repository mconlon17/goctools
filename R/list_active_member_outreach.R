...
#'
#' @examples
#' list_active_member_outreach()
list_active_member_outreach <- function() {
  ...
  contacts <- get_contacts() %>%
    ...
    dplyr::inner_join(get_attendance(days = 90), by = c("id" = "contact_2_id")) %>%
  ...
  ft <- goc_table(contacts, paste("Active Member Outreach for", Sys.Date()))
  ft
}
