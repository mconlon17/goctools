...
plot_membership_by_the_numbers <- function(date=Sys.Date(), months=15) {
    ...
    p1  <- goc_plot(ggplot(data, aes(x=Month, y=`Unique Members`)) + geom_line())
    p2  <- goc_plot(ggplot(data, aes(x=Month, y=`Average Workday Hours`)) + geom_line())
    p3  <- goc_plot(ggplot(data, aes(x=Month, y=`Average Daily Attendance`)) + geom_line())
    p4  <- goc_plot(ggplot(data, aes(x=Month, y=`New Members`)) + ggplot2::geom_col())
    p5  <- goc_plot(ggplot(data, aes(x=Month, y=`Supports`)) + geom_line())
    p6  <- goc_plot(ggplot(data, aes(x=Month, y=`Members with Supports`)) + geom_line())
    p7  <- goc_plot(ggplot(data, aes(x=Month, y=`Pre-employment Supports`)) + ggplot2::geom_col())
    p8  <- goc_plot(ggplot(data, aes(x=Month, y=`TE Supports`)) + ggplot2::geom_col())
    p9  <- goc_plot(ggplot(data, aes(x=Month, y=`SE Supports`)) + ggplot2::geom_col())
    p10 <- goc_plot(ggplot(data, aes(x=Month, y=`IE Supports`)) + ggplot2::geom_col())
    p11 <- goc_plot(ggplot(data, aes(x=Month, y=`Donors`)) + geom_line())
    p12 <- goc_plot(ggplot(data, aes(x=Month, y=`Amount Raised`)) + geom_line())
    
    ((p1 | p2  | p3  | p4 ) /
     (p5 | p6  | p7  | p8 ) /
     (p9 | p10 | p11 | p12 )) + patchwork::plot_annotation(title = paste0('GOC monthly data from ', 
